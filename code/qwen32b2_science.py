...
def generate_response(prompt, system_prompt=None, max_new_tokens=64, temperature=0.1, top_p=0.9):
    ...
    client = OpenAI(
        ...
        api_key='2600c3ff-b766-45b7-a0d7-0836c0663a48',  # ModelScope Token
    )
...
if __name__ == "__main__":
    ...
    for subject in ['Science']:  # 只处理Science类别'Art','Science'
        print(subject)
        for i in range(1, 11):  # 处理1-10号文件
            print(i)
            for prompt_version in range(2, 3):  # 测试3种提示版本
                print("Prompt版本:", prompt_version)
                file_name = f"{subject}{i}.xlsx"
                excel_file = os.path.join(input_folder, file_name)

                output_file = f"output1/qwen32b/Result{subject}{i}_prompt{prompt_version}_qwen32b.xlsx"
