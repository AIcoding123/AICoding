...
if __name__ == "__main__":
    ...
    for subject in ['Science']:  # 只处理Science类别'Art',
        print(subject)
        for i in range(6, 11):  # 处理1-10号文件
            print(i)
            for prompt_version in range(1, 4):  # 测试3种提示版本
                print("Prompt版本:", prompt_version)
                file_name = f"{subject}{i}.xlsx"
                excel_file = os.path.join(input_folder, file_name)

                output_file = f"output1/qwen2.5/Result{subject}{i}_prompt{prompt_version}_qwen2.5.xlsx"
