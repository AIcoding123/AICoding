...
def generate_response(prompt, system_prompt=None, max_new_tokens=64, temperature=0.1, top_p=0.9):
    ...
    client = OpenAI(
        ...
        api_key='9e0e9a34-c5cd-4452-8d82-4a5d3cc207af',  # ModelScope Token
    )
...
if __name__ == "__main__":
    ...
    for subject in ['Science']:#'Art',,'Science','Science'
        print(subject)
        for i in range(1, 11):
            print(i)
            for prompt_version in range(1, 2):  # 测试所有3种提示版本
                print("Prompt版本:", prompt_version)
                file_name = f"{subject}{i}.xlsx"
                excel_file = os.path.join(input_folder, file_name)

                # 创建输出文件路径
                output_file = f"output1/slidingqwen32b/Result{subject}{i}_prompt{prompt_version}_qwen32b.xlsx"
