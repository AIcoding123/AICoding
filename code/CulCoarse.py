...
# 读取 Excel 文件
data = pd.read_excel('EvaluateDataset/Qwen2-7B-Instructvoting/ALL_Vote.xlsx')
print(data.head())

# 清除包含空白的行
data.dropna(subset=['Coarsegrained', 'IFIAS_Code'], inplace=True)

# 删除 y_pred 不是 0, 1, 2 的行
data = data[data['IFIAS_Code'].isin([0, 1, 2])]

# 获取真实标签和预测标签
y_true = data['Coarsegrained']
y_pred = data['IFIAS_Code'].astype(int)
