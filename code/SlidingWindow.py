...
def generate_response(prompt, system_prompt=None, max_new_tokens=64, temperature=0.1, top_p=0.9):
    ...
    headers = {
        "Authorization": "Bearer sk-psxspuuwacnnlezwuktzbvevyerrhrncqzrepaukfojrvthy",  # 替换为您的API密钥#sk-rzducwpolxaqwzuzaniroxxpeybodttfqnhlwtjfnczqlqgm
...
def create_ifias_prompt(code2,classroom_text, classroom_text_shangwen, classroom_text_xiawen, ifias_framework):
    """
    创建IFIAS编码提示
    参数:
...
def process_excel_file(excel_file, text_column,text_column1, batch_size=1,j=1, output_file=None):
    """
    ...
    for i in range(len(df[text_column])):
        ...
        code2 = df[text_column1].iloc[i]  # 获取当前文本
        print("text", text)
        ...
        if pd.notna(text):  # 处理NaN值
            ...
            # 创建提示
            prompt = create_ifias_prompt(code2,text, classroom_text_shangwen_str, classroom_text_xiawen_str, ifias_framework)
...
if __name__ == "__main__":
    ...
    for subject in ['Art','Science']:#'Art',,'Science'
        print(subject)
        for i in range(1, 11):#(8, 9)
            print(i)
            for j in range(1, 4):#(3, 4)
                print(j)
                ...
                # 创建包含时间戳的文件路径
                output_file = f"output1/slidingglm4/Result{subject}{i}prompt{j}glm4.xlsx"
                # 包含课堂对话的列名
