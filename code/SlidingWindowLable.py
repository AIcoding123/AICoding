...
def generate_response(prompt, system_prompt=None, max_new_tokens=64, temperature=0.1, top_p=0.9):
    ...
    headers = {
        ...
        # "Authorization": "Bearer sk-rzducwpolxaqwzuzaniroxxpeybodttfqnhlwtjfnczqlqgm",
    ...
    payload = {
        "model": "THUDM/glm-4-9b-chat",#Qwen/Qwen2-7B-Instruct#THUDM/glm-4-9b-chat
        "messages": messages,
...
def create_ifias_prompt(classroom_text, classroom_text_shangwen, classroom_text_xiawen, ifias_framework):
    """
    创建IFIAS编码提示
    参数:
    ...
    prompt = f"""你是一位资深课堂教研专家，请根据IFIAS编码框架对课堂内容进行编码。仅需要给出粗粒度的编码结果，即教师语言为0，学生语言为1，技术为2  
        ...
        {ifias_framework}  
        不是输出Python代码。仅需要判断我现在给你的。请将下面的课堂文本进行编码。以<code>数字</code>格式回复，格式如下：<code>0</code>或<code>1</code>或<code>2</code>。  
        你只需要输出编码结果，不要包括其他内容。  
        上下文会被放在<context></context>中，要进行编码的课堂内容文本会被放在<text2code></text2code>中。  

        请严格按照以下步骤执行任务：  
        1. 完整阅读<context>包含的课堂内容上下文及其对应的标签<code></code></context>  
    ...
    for i in range(len(df[text_column])):
        ...
        if pd.notna(text):  # 处理NaN值
            ...
            if i >= 2:  # 确保当前索引i至少为2
                ...
                classroom_shangwen_labels = df['Coarsegrained'].iloc[i - 2:i].tolist()
            else:  # 如果i小于2，获取所有可能的文本和标签
                classroom_text_shangwen = df[text_column].iloc[:i].tolist()
                classroom_shangwen_labels = df['Coarsegrained'].iloc[:i].tolist()

                # 获取后两个文本的标签
            if i + 2 < len(df[text_column]):  # 确保当前索引+2没有超出范围
                classroom_text_xiawen = df[text_column].iloc[i + 1:i + 3].tolist()
                classroom_xiawen_labels = df['Coarsegrained'].iloc[i + 1:i + 3].tolist()
            else:  # 如果超出范围，则获取剩余的文本和标签
                classroom_text_xiawen = df[text_column].iloc[i + 1:].tolist()
                classroom_xiawen_labels = df['Coarsegrained'].iloc[i + 1:].tolist()

                # 合并前后文本和标签
            classroom_text_shangwen_str = ' '.join(
                f"{text} 这句编码是<code>{label}</code>" for text, label in
                zip(classroom_text_shangwen, classroom_shangwen_labels)
            )

            classroom_text_xiawen_str = ' '.join(
                f"{text} 这句编码是<code>{label}</code>" for text, label in zip(classroom_text_xiawen, classroom_xiawen_labels)
            )

            # 创建提示
            prompt = create_ifias_prompt(text, classroom_text_shangwen_str, classroom_text_xiawen_str, ifias_framework)
...
if __name__ == "__main__":
    ...
    for subject in ['Art','Science']:#'Art',
        print(subject)
        for i in range(1, 11):
            print(i)
            for j in range(1, 4):
                ...
                # 创建包含时间戳的文件路径
                output_file = f"output/slidingLable11glm4/Result{subject}{i}prompt{j}glm4.xlsx"
                # 包含课堂对话的列名
